...
> You can also deploy from staged files with `eb deploy --staged` if you don't want to commit the changes yet and `eb status` will get you info about your enviroment.
...
### `eb ssh` and understanding what's going on [Optional].

> if you are in a hurry and have a deployment deadline in an hour, then skip this part .. actually .. shouldn't you be just reading the the TL;DR section ? this is a good time to reflect on your life choices
...
> eb will run the `container_commands` in **"alphabetical order by name"** this is why we have the `01` so that if we ever needed to run any other command we can decide which runs first.
